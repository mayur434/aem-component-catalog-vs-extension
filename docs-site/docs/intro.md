...
- **Cross-brand duplicate detection.** The companion Tech Audit Report (Excel output) scans component source across reactors — including legacy AMS projects, not just AEMaaCS — to surface duplicated components, a governance question the per-reactor catalog view alone can't answer.
...
- Support for **AEMaaCS only** for catalog generation. AEM 6.5, Adobe Managed Services, and on-premise projects are intentionally unsupported by the generator (the separate Audit/duplicate-detection report is the one feature that still reads AMS projects).
